...
const FooterSection = () => {
  return (
    <section className="w-full max-w-screen-2xl px-[1rem] md:px-14 mx-auto flex flex-col md:flex-row items-start gap-10 mt-10 md:mt-20">
      <div className="md:w-[60%] w-full flex gap-6 md:justify-between">
        ...
        <div className="flex items-center md:gap-16 gap-4">
            <div className="flex flex-col gap-3 items-start">
                {footerItemsLinks.slice(0, 4).map((item) => (
                    <Link href={item.link} key={item.title} className="text-customGray text-sm hover:border-b-[2px] border-customGray">
                        {item.title}
                    </Link>
                ))}
            </div>
      ...
      <div className="w-full h-[30vh] md:h-[40vh] 2xl:h-[30vh] rounded-lg overflow-hidden bg-white border">
              <iframe
